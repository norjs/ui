...
class NrNavController {
    ...
    onClick (item) {
        ...
        if ( this._options && this._options.onClick !== undefined ) {

            if (_.isFunction(this._options.onClick)) {

                nrLog.trace(`.onClick(): Calling options.onClick()"`);

                this._options.onClick(item);

            } else {

                throw new TypeError(`${nrLog.name}.onClick(): options.onClick was not a function: ${LogUtils.getAsString(this._options.onClick)}`);

            }

        } else {

            nrLog.trace(`.onClick(): No options.onClick configured; emitting an event NAV_CLICK "${NrEventName.NAV_CLICK}"`);

            this._$scope.$emit(NrEventName.NAV_CLICK, item, this);
